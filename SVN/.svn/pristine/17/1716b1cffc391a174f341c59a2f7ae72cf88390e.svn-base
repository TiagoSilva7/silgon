...
def main():
    ...
    url_rest_api_desenv = 'http://10.14.203.158:8080/SEFADEVLIB/api/'
    ...
    response = client_session.post(url_rest_api_desenv + 'auth/login', headers=headers, json=json_data)
    ...
    mstr_group_ids_names = client_session.get(url_rest_api_desenv + 'usergroups', params=group_params, headers=new_header)
    ...
    todos_usuarios_mstr = client_session.get(url_rest_api_desenv + 'users', params=users_params, headers=new_header)
    ...
    for id_grupos_deleta_usuarios_mstr in mstr_groups_ids:
        ...
        #Desenvolvimento
        users_ids_response = client_session.get(url_rest_api_desenv + 'usergroups/'+id_grupos_deleta_usuarios_mstr+'/members', params=users_params, headers=new_header)
        ...
        remove_usuarios_mstr = client_session.patch(url_rest_api_desenv + 'usergroups/'+id_grupos_deleta_usuarios_mstr, headers=new_header, json=json_data_remove)    
        ...
        #remove_usuarios_mstr = client_session.patch(url_rest_api_prod + 'sergroups/'+id_grupos_deleta_usuarios_mstr, headers=new_header, json=json_data_remove)    
    ...
    for grupo_a_ser_adicionado, usuario_a_Ser_Adicionado in usuarios_para_adicionar_no_mstr.items():
        ...
        status_adicao = client_session.patch(url_rest_api_desenv + 'usergroups/'+grupo_a_ser_adicionado, headers=new_header, json=json_data_add)     
    ...
    for status_usuario_teradata in teradata_status_usuarios:
        ...
        for usuario_microstrategy_full in usuarios_mstr:
            ...
            if status_usuario_teradata[0] == usuario_microstrategy_full['abbreviation']:
                ...
                status_alteracao_usuario = client_session.patch(url_rest_api_desenv + 'users/'+usuario_microstrategy_full['id'], headers=new_header, json=json_data_alter)
    ...
    for desativa_usuarios_sem_grupo in lista_id_usuarios_mstr:
        #Desenvolvimento
        usuarios_grupo_mstr_all = client_session.get(url_rest_api_desenv + 'users/'+desativa_usuarios_sem_grupo, params=users_params_groups, headers=new_header)
        #Produção
    ...
    for usuarios_desativar_sem_grupo in usuarios_desativar:
        ...
        #Desenvolvimento
        status_alteracao_usuario_sem_grupo = client_session.patch(url_rest_api_desenv + 'users/'+usuarios_desativar_sem_grupo, headers=new_header, json=json_data_alter_sem_grupo)
        #Produção
