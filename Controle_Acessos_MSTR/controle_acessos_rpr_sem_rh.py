...
def main():
    ...
    with teradatasql.connect(host='10.14.203.21', user='tiago.silva', password='sefacre') as connect:
        teradata_table = pd.read_sql("""SELECT DISTINCT 
                                            ...
                                            FROM
                                                ...
                                                ) A""".replace("$DB$", json_var["TD_AMB"]), connect)
        ...
        with teradatasql.connect(host='10.14.203.21', user='tiago.silva', password='sefacre') as connect:
         teradata_stts_users = pd.read_sql("""SELECT DISTINCT 
                                                ...
                                                FROM
                                                    $DB$_TGTDB.ERL_STATUS_USUAR_RPR SUR
                                                    JOIN $DB$_TGTDB.ERL_TR_GRUPO_USUAR_RPR GUR
                                                        ON (SUR.CD_GRUPO_RPR = GUR.CD_GRUPO_RPR)""".replace("$DB$", json_var["TD_AMB"]), connect)
    ...
    #print(usuarios_mstr[0]['id'])

    lista_id_usuarios_mstr = []

    for usuario_distinto_microstrategy in usuarios_mstr:
        #print(usuario_distinto_microstrategy['id'])
        lista_id_usuarios_mstr.append(usuario_distinto_microstrategy['id'])
        # print(usuario_distinto_microstrategy)

    #print(lista_id_usuarios_mstr)
    #print(lista_id_usuarios_mstr)

    usuarios_desativar = []

    for remover_usuarios in lista_id_usuarios_mstr:
        usuarios_grupo_mstr_all = client_session.get('http://10.14.203.158:8080/SEFADEVLIB/api/users/'+remover_usuarios, params=users_params_groups, headers=new_header)
    #8CAC30BC453839CBC243DE89BE4B8667 - RODRIGO ALBERTO DE OLIVEIRA - 3704222950
    #BD6D58C44B69AB9B6C3AAB958073DB5B - Tigo
        xpto = usuarios_grupo_mstr_all.json()

        my_list = [i for i in xpto.values()]
     
        if len(my_list[1]) == 1:
            if my_list[1][0]['name'] == 'Everyone':
                usuarios_desativar.append(my_list[0])
        
    #print(usuarios_desativar)

  


    #    descricao_completa['abbreviation'] == str('31071655817')
    #    print(descricao_completa['name'])
    ...
    for usuarios_desativar_sem_grupo in usuarios_desativar:
        #print(usuarios_desativar_sem_grupo)
        ...
        #print(id_usuario_alterado_sem_grupo, status_usuario_alterado_sem_grupo)
