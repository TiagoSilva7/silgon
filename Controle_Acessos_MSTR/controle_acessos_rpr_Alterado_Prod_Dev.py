...
def main():
    ...
    with udaExec.connect(
        ...
        #                                         JOIN $DB$_ACCDB.DIM_RH_FUNC RHF 
        ...
        #                                     FROM 
        #                                         (SELECT 
        #                                             COALESCE(LPAD(CAST(SUR.NU_CPF_USUAR_RPR as VARCHAR(11)), 11, '0'), 
        #                                             LPAD(CAST(RHF.NU_CPF as VARCHAR(11)), 11, '0')) AS NU_CPF_USUAR_RPR,
        #                                             LPAD(CAST(SUR.CD_GRUPO_RPR as VARCHAR(3)), 3, '0') as CD_GRUPO_RPR,
        #                                             GUR.NM_GRUPO_RPR,
        #                                             SUR.DS_STATUS_USUAR_RPR, 
        #                                             CASE WHEN RHF.CD_SIT_FUNC = 1 THEN 'true' ELSE 'false' END AS DS_STATUS_RH 
        #                                         FROM 
        #                                             $DB$_TGTDB.ERL_STATUS_USUAR_RPR SUR 
        #                                             RIGHT JOIN $DB$_ACCDB.DIM_RH_FUNC RHF 
        #                                                 ON (SUR.NU_CPF_USUAR_RPR = RHF.NU_CPF) 
        #                                             JOIN $DB$_ACCDB.DIM_RH_SIT_FUNC RHSF 
        #                                                 ON (RHF.CD_SIT_FUNC = RHSF.CD_SIT_FUNC) 
        #                                             LEFT JOIN $DB$_TGTDB.ERL_TR_GRUPO_USUAR_RPR GUR 
        #                                                 ON (SUR.CD_GRUPO_RPR = GUR.CD_GRUPO_RPR) 
    ...
    for id_grupos_deleta_usuarios_mstr in mstr_groups_ids:
        ...
        #Produção
        users_ids_response = client_session.get('http://10.14.203.158:8080/SEFAPRODLIB_TESTE/api/usergroups/'+id_grupos_deleta_usuarios_mstr+'/members', params=users_params, headers=new_header)
        ...
        remove_usuarios_mstr = client_session.patch('http://10.14.203.158:8080/SEFAPRODLIB_TESTE/api/usergroups/'+id_grupos_deleta_usuarios_mstr, headers=new_header, json=json_data_remove)    
    ...
    for grupo_a_ser_adicionado, usuario_a_Ser_Adicionado in usuarios_para_adicionar_no_mstr.items():
        ...
        status_adicao = client_session.patch('http://10.14.203.158:8080/SEFAPRODLIB_TESTE/api/usergroups/'+grupo_a_ser_adicionado, headers=new_header, json=json_data_add)     
    ...
    for status_usuario_teradata in teradata_status_usuarios:
        ...
        for usuario_microstrategy_full in usuarios_mstr:
            ...
            if status_usuario_teradata[0] == usuario_microstrategy_full['abbreviation']:
                ...
                status_alteracao_usuario = client_session.patch('http://10.14.203.158:8080/SEFAPRODLIB_TESTE/api/users/'+usuario_microstrategy_full['id'], headers=new_header, json=json_data_alter)
    ...
    for desativa_usuarios_sem_grupo in lista_id_usuarios_mstr:
        #Desenvolvimento
        #usuarios_grupo_mstr_all = client_session.get('http://10.14.203.158:8080/SEFADEVLIB/api/users/'+desativa_usuarios_sem_grupo, params=users_params_groups, headers=new_header)
        #Produção
        usuarios_grupo_mstr_all = client_session.get('http://10.14.203.158:8080/SEFAPRODLIB_TESTE/api/users/'+desativa_usuarios_sem_grupo, params=users_params_groups, headers=new_header)
    ...
    for usuarios_desativar_sem_grupo in usuarios_desativar:
        ...
        #Desenvolvimento
        #status_alteracao_usuario_sem_grupo = client_session.patch('http://10.14.203.158:8080/SEFADEVLIB/api/users/'+usuarios_desativar_sem_grupo, headers=new_header, json=json_data_alter_sem_grupo)
        #Produção
        status_alteracao_usuario_sem_grupo = client_session.patch('http://10.14.203.158:8080/SEFAPRODLIB_TESTE/api/users/'+usuarios_desativar_sem_grupo, headers=new_header, json=json_data_alter_sem_grupo)
